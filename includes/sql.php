...
function find_by_violentados($tableA, $tableB, $id, $tipo)
{
  global $db;
  $id = (int)$id;
  $sql = "SELECT id_{$db->escape($tableB)} ,IFNULL(descripcion,'')as descripcion 
...
/*
Obtengo todos los derechos violentados sin importar reclasificación
*/
function find_by_violentados_all($tableA, $tableB, $id)
{
  global $db;
  $id = (int)$id;
  $sql = "SELECT id_{$db->escape($tableB)} as id_campo,IFNULL(descripcion,'')as descripcion , id_rel_queja_reclasificacion
	FROM {$db->escape($tableA)} 
	LEFT JOIN {$db->escape($tableB)} 
	USING(id_{$db->escape($tableB)}) 
	WHERE id_queja_date='{$db->escape($id)}' ";
...
/*
Obtengo todos los derechos violentados historicos sin importar reclasificación
*/
function find_by_violentados_old($tableA, $tableB, $id)
{
  global $db;
  $id = (int)$id;
  $sql = "SELECT id_{$db->escape($tableB)} as id_campo,IFNULL(descripcion,'')as descripcion , id_rel_queja_reclasificacion
...
function find_all_quejas($id, $user, $anio)
{
  global $db;
  $id = (int)$id;
  $sql = "SELECT q.id_queja_date, q.folio_queja, q.fecha_presentacion, q.id_cat_med_pres,mp.descripcion as medio_pres, au.nombre_autoridad, 
  cq.nombre as nombre_quejoso,cq.paterno as paterno_quejoso, cq.materno as materno_quejoso, CONCAT(ca.`nombre`,' ',ca.`paterno`,' ',ca.`materno`) as nombre_agraviado,
  IFNULL(fecha_estado_procesal,date_format(fecha_presentacion, '%d-%m-%Y')) as fecha_estado_procesal,q.id_tipo_resolucion as id_resolucion,
  ep.descripcion as est_proc, ctr.descripcion as id_tipo_resolucion,ar.nombre_area, CONCAT(du.nombre,' ',du.apellidos) as user_asignado,q.estado_procesal,fecha_actualizacion,q.fecha_creacion ,
  ...
  $sql .= " LEFT JOIN cat_agraviados ca ON ca.id_cat_agrav = q.id_cat_agraviado";
  ...
  $sql .= " LEFT JOIN (  	SELECT COUNT(id_queja_date) as total_der_vul,id_queja_date FROM rel_queja_der_vuln  GROUP BY id_queja_date  )x ON(q.id_queja_date =x.id_queja_date)
  ...
  $sql .= " AND q.folio_queja LIKE '%/{$anio}-%'  ORDER BY q.id_queja_date DESC";
  $result = find_by_sql($sql);
  return $result;
}
...
function find_all_quejas_region($anio, $id_area)
{
  ...
  $sql .= " cq.paterno as paterno_quejoso, cq.materno as materno_quejoso, CONCAT(ca.`nombre`,' ',ca.`paterno`,' ',ca.`materno`) as nombre_agraviado,q.fecha_creacion,IFNULL(fecha_estado_procesal,date_format(fecha_presentacion, '%d-%m-%Y')) as fecha_estado_procesal, 
  q.archivo, ep.descripcion as est_proc, ctr.descripcion as id_tipo_resolucion,ar.nombre_area, CONCAT(du.nombre,' ',du.apellidos) as user_asignado,fecha_actualizacion,IF((IFNULL(total_der_gral,0)+IFNULL(total_der_vul,0)) >0,'Si','No')  calificado ";
  $sql .= " FROM quejas_dates q";
  $sql .= " LEFT JOIN cat_medio_pres mp ON mp.id_cat_med_pres = q.id_cat_med_pres";
  $sql .= " LEFT JOIN cat_autoridades au ON au.id_cat_aut = q.id_cat_aut";
  $sql .= " LEFT JOIN cat_quejosos cq ON cq.id_cat_quejoso = q.id_cat_quejoso";
  ...
  $sql .= " LEFT JOIN (  	SELECT COUNT(id_queja_date) as total_der_vul,id_queja_date FROM rel_queja_der_vuln  GROUP BY id_queja_date  )x ON(q.id_queja_date =x.id_queja_date)
			LEFT JOIN (  	SELECT COUNT(id_queja_date) as total_der_gral,id_queja_date FROM rel_queja_der_gral      GROUP BY id_queja_date  )y ON(q.id_queja_date =y.id_queja_date) ";
  $sql .= " WHERE q.id_area_asignada = {$id_area}";
...
function find_all_quejas_mor($anio)
{
  $sql = "SELECT q.id_queja_date, q.folio_queja, q.fecha_presentacion, q.id_cat_med_pres, mp.id_cat_med_pres, mp.descripcion as medio_pres, au.nombre_autoridad, 
  cq.nombre as nombre_quejoso,id_user_asignado,q.estado_procesal, q.id_tipo_resolucion as id_resolucion, ";
  $sql .= " cq.paterno as paterno_quejoso, cq.materno as materno_quejoso, CONCAT(ca.`nombre`,' ',ca.`paterno`,' ',ca.`materno`) as nombre_agraviado,q.fecha_creacion,IFNULL(fecha_estado_procesal,date_format(fecha_presentacion, '%d-%m-%Y')) as fecha_estado_procesal, 
  ...
  $sql .= " LEFT JOIN (  	SELECT COUNT(id_queja_date) as total_der_vul,id_queja_date FROM rel_queja_der_vuln  GROUP BY id_queja_date  )x ON(q.id_queja_date =x.id_queja_date)
			LEFT JOIN (  	SELECT COUNT(id_queja_date) as total_der_gral,id_queja_date FROM rel_queja_der_gral      GROUP BY id_queja_date  )y ON(q.id_queja_date =y.id_queja_date) ";
  $sql .= " WHERE q.id_area_asignada = 24";
  $sql .= " AND q.folio_queja LIKE '%/{$anio}-%' ";
  $result = find_by_sql($sql);
  return $result;
}
function find_all_quejas_admin($anio)
{
  ...
  $sql .= " LEFT JOIN cat_tipo_res ctr ON ctr.id_cat_tipo_res = q.id_tipo_resolucion ";
  $sql .= " LEFT JOIN `area` ar ON q.`id_area_asignada` = ar.id_area  ";
  $sql .= " LEFT JOIN users u ON u.id_user = q.id_user_creador  ";
  $sql .= " LEFT JOIN `detalles_usuario` du ON q.`id_user_asignado` =  du.id_det_usuario";
  $sql .= " LEFT JOIN (
 	SELECT COUNT(id_queja_date) as total_der_vul,id_queja_date FROM rel_queja_der_vuln 
    GROUP BY id_queja_date
 )x ON(q.id_queja_date =x.id_queja_date)
LEFT JOIN (
 	SELECT COUNT(id_queja_date) as total_der_gral,id_queja_date FROM rel_queja_der_gral 
    GROUP BY id_queja_date
 )y ON(q.id_queja_date =y.id_queja_date) ";
...
/*----------------------------------------------------------*/
/* Funcion para encontrar si existe el nombre de usuario    */
/*----------------------------------------------------------*/
...
function find_all_cuentas()
{
  ...
  $sql .= "g.nombre_grupo,d.id_area,nombre_area ";
  $sql .= "FROM users u ";
  $sql .= "LEFT JOIN detalles_usuario d ON d.id_det_usuario = u.id_detalle_user ";
  $sql .= "LEFT JOIN grupo_usuarios g ON g.nivel_grupo=u.user_level ";
  $sql .= "LEFT JOIN `area` e ON(d.id_area= e.id_area) ";
...
function find_all_canalizaciones($anio)
{
  ...
  // $sql .= " LEFT JOIN cat_municipios as mp ON mp.id_cat_mun = o.id_cat_mun ";
  $sql .= " LEFT JOIN detalles_usuario as d ON d.id_det_usuario = u.id_detalle_user WHERE tipo_solicitud=2 AND o.folio LIKE '%/{$anio}-%' ORDER BY o.id_or_can DESc  ;";
  return find_by_sql($sql);
}
...
/*----------------------------------------------*/
function find_all_actuaciones($anio)
{
  global $db;
  $results = array();
  $sql = "SELECT a.*, r.nombre_autoridad as estatal, ra.nombre_autoridad as federal, CONCAT(IFNULL(d.nombre,''),' ',IFNULL(d.apellidos,'')) as nombre_creador 
  ...
  LEFT JOIN users as u ON u.id_user = a.user_creador
   LEFT JOIN detalles_usuario as d ON d.id_det_usuario = u.id_detalle_user
  WHERE  folio_actuacion LIKE '%/{$anio}-%' ORDER BY a.folio_actuacion DESc ";
  $result = find_by_sql($sql);
  return $result;
}
...
function find_all_actuaciones_area($anio, $area)
{
  global $db;
  $results = array();
  //$sql = "SELECT * FROM actuaciones WHERE area_creacion = '{$area}' AND folio_actuacion LIKE '%/{$anio}-%' ORDER BY fecha_captura_acta";
  $sql = "SELECT a.*, r.nombre_autoridad as estatal, ra.nombre_autoridad as federal, CONCAT(IFNULL(d.nombre,''),' ',IFNULL(d.apellidos,'')) as nombre_creador 
  FROM actuaciones as a 
  LEFT JOIN cat_autoridades as r ON (a.autoridades = r.id_cat_aut)
  LEFT JOIN cat_autoridades as ra ON (a.autoridades_federales = ra.id_cat_aut) 
...
function find_by_id_orientacion($id)
{
  ...
  $sql = $db->query("SELECT o.id_or_can as idcan,o.folio,o.correo_electronico,o.nombre_completo,cesc.descripcion as cesc,ocup.descripcion as ocup,o.edad,
                      ...
                      mp.descripcion as municipio,o.id_or_can
...
{
  global $db;
  $sql  = "SELECT id_rel_queja_acuerdos,tipo_acuerdo,
			fecha_acuerdo,acuerdo_adjunto,acuerdo_adjunto_publico,
			sintesis_documento,publico,origen_acuerdo,
			IFNULL(documento_promocion,'') as documento_promocion,
			IFNULL(id_cat_tipo_res,0) as id_cat_tipo_res,
            REPLACE(REPLACE(b.descripcion,'/','_'),' ','_') as nombre_resolucion,
            IFNULL(observaciones_resolucion,'') as observaciones_resolucion
			FROM `rel_queja_acuerdos` 
			LEFT JOIN cat_tipo_res b USING(id_cat_tipo_res) 
			WHERE id_queja_date= {$id} ORDER By fecha_acuerdo ASC";
  return $db->query($sql);
}
...
function find_all_consejo($year)
{
  global $db;
  $results = array();
  $sql = "SELECT * FROM consejo WHERE YEAR(fecha_sesion) ='{$year}' ORDER BY fecha_sesion";
  $result = find_by_sql($sql);
  return $result;
}
/*----------------------------------------------------------------------------------*/
...
function find_by_id_actuacion($id)
{
  global $db;
  $id = (int)$id;
  $sql = $db->query("SELECT a.*,CONCAT(IFNULL(d.nombre,''),' ',IFNULL(d.apellidos,'')) as nombre_creador  FROM actuaciones a LEFT JOIN users as u ON u.id_user = a.user_creador
   LEFT JOIN detalles_usuario as d ON d.id_det_usuario = u.id_detalle_user WHERE id_actuacion='{$db->escape($id)}' LIMIT 1");
  if ($result = $db->fetch_assoc($sql))
    return $result;
  else
    return null;
...
function derechoVulneradoQ($year)
{
...
/*--------------------------------------------*/
function find_all_convenios($year)
{
  //$sql = "SELECT * FROM convenios WHERE folio_solicitud LIKE '%/{$year}-%' ORDER BY folio_solicitud DESC";
  $sql = "SELECT * FROM convenios WHERE YEAR(fecha_convenio) ='{$year}' ORDER BY fecha_convenio DESC";
  $result = find_by_sql($sql);
  return $result;
}
...
{
  $sql = "SELECT a.*,b.`nombre_area`, c.nombre_area as nombre_area_creacion FROM correspondencia a LEFT JOIN `area` b ON a.`id_area_turnada`=b.`id_area` LEFT JOIN `area` c ON a.`area_creacion`=c.`id_area` ORDER BY fecha_recibido DESC";
  $result = find_by_sql($sql);
  return $result;
}
/*-------------------------------------------------------------*/
...
/* Funcion que encuentra todas las orientaciones */
/*----------------------------------------------*/
function find_all_capacitaciones($ejercicio)
{
  global $db;
  $results = array();
  $sql = "SELECT * FROM capacitaciones WHERE YEAR(fecha)='{$ejercicio}' ORDER BY fecha";
  $result = find_by_sql($sql);
  return $result;
}
...
function find_all_capacitaciones_area($area, $ejercicio)
{
  global $db;
  $results = array();
  $sql = "SELECT * FROM capacitaciones WHERE area_creacion = '{$area}' AND  YEAR(fecha)='{$ejercicio}' ORDER BY fecha";
  $result = find_by_sql($sql);
  return $result;
}
...
/* Funcion que encuentra todas las recomendaciones */
/*-------------------------------------------------*/
function find_all_recomendaciones($ejercicio)
{
  $sql = "SELECT * FROM recomendaciones WHERE numero_recomendacion LIKE '%/{$ejercicio}'";
  $result = find_by_sql($sql);
  return $result;
}
...
{
  global $db;
  $query = "SELECT id_user, id_detalle_user	, nombre, apellidos, id_cat_gen, id_cargo, nombre_cargo, d.id_area, nombre_area    
FROM  users b  
...
/* Funcion que encuentra todas las correspondencias */
/*--------------------------------------------------*/
function find_all_env_correspondenciaAdmin($year)
{
  $sql = "SELECT c.id_env_corresp, IFNULL(no_oficio,'S/N') as no_oficio,c.folio, IFNULL(c.fecha_en_que_se_turna,'') as fecha_en_que_se_turna, tipo_tramite,
...
function find_by_id_env_correspondencia($id)
{
  ...
  $sql = $db->query("SELECT c.id_env_corresp, c.folio, c.fecha_emision, c.tipo_tramite, c.oficio_enviado, c.fecha_en_que_se_turna, c.fecha_espera_respuesta, 
                            ...
                            c.area_creacion, c.fecha_creacion, c.user_creador, c.se_turna_a_area, d.nombre, d.apellidos, atu.nombre_area as nomat,no_oficio,IF(accion_realizada is null,'0','1') as atendida 
                    FROM envio_correspondencia c
                    LEFT JOIN area a ON c.se_turna_a_area = a.id_area 
...
/* Funcion que encuentra todas las correspondencias */
/*--------------------------------------------------*/
function find_all_env_correspondenciaAdmin2($year)
{
  $sql = "SELECT c.id_env_corresp, c.folio, IFNULL(no_oficio,'S/N') as no_oficio, IFNULL(c.fecha_en_que_se_turna,'') as fecha_en_que_se_turna, 
  IFNULL(c.fecha_espera_respuesta,'') as fecha_espera_respuesta, c.asunto, c.medio_envio, a.nombre_area,IF(accion_realizada is null,'No','Si') as accion_realizada, 
  IF(accion_realizada is null,'0','1') as atendida,b.nombre_area as nombre_area_creacion  
          FROM envio_correspondencia c
          LEFT JOIN area a ON c.se_turna_a_area = a.id_area 
		  LEFT JOIN area b ON c.area_creacion = b.id_area ORDER BY fecha_en_que_se_turna DESC";
  $result = find_by_sql($sql);
  return $result;
}
/*-------------------------------------------------------------*/
...
function find_all_fichas($anio)
{
  $sql = "SELECT f.id_ficha,
       f.folio,
       fun.descripcion as funcion,
       ff.folio as num_queja,
...
WHERE tipo_ficha = 1 AND  YEAR(fecha_intervencion)= '{$anio}' ;";
  $result = find_by_sql($sql);
  return $result;
}
/*----------------------------------------------------------------*/
...
function find_all_fichasUser($anio, $id_user)
{
  ...
  $sql = "SELECT f.id_ficha,
       ...
       fun.descripcion as funcion,
       ff.folio as num_queja,
	   IFNULL(protocolo_estambul,'') as protocolo_estambul,
       CONCAT(p.nombre,' ', p.paterno,' ', p.materno) as nombre_paciente,
       f.ficha_adjunto,
       a2.nombre_area as area_solicitante
FROM fichas f
...
LEFT JOIN paciente p ON p.id_paciente = f.id_paciente
LEFT JOIN folios ff ON ff.id_folio = p.folio_expediente
/*WHERE tipo_ficha = 1 AND user_creador = '{$id_user}' AND f.folio LIKE '%/{$anio}-%' ;*/
WHERE tipo_ficha = 1 AND user_creador = '{$id_user}' AND YEAR(fecha_intervencion)= '{$anio}' ;";
  $result = find_by_sql($sql);
  return $result;
}
...
function find_by_id_ficha($id_ficha, $tipo)
{
  ...
  $sql = $db->query("SELECT f.id_ficha, f.folio, fun.descripcion as funcion, ff.folio as num_queja,
       CONCAT(p.nombre,' ', p.paterno,' ', p.materno) as nombre_paciente, f.ficha_adjunto, 
		...
		IFNULL(protocolo_estambul,'') as protocolo_estambul,fecha_intervencion,resultado,documento_emitido,
...
LEFT JOIN paciente p ON p.id_paciente = f.id_paciente
LEFT JOIN folios ff ON ff.id_folio = p.folio_expediente
WHERE tipo_ficha = 2  AND YEAR(fecha_intervencion)= '{$anio}' ;";
  $result = find_by_sql($sql);
  return $result;
}
/*----------------------------------------------------------------*/
...
LEFT JOIN paciente p ON p.id_paciente = f.id_paciente
LEFT JOIN folios ff ON ff.id_folio = p.folio_expediente
WHERE tipo_ficha = 2 AND user_creador = '{$id_user}' AND YEAR(fecha_intervencion)= '{$anio}' ;";
  $result = find_by_sql($sql);
  return $result;
}
...
function find_all_entregables()
{
  global $db;
  $results = array();
  $sql = "SELECT  id_entregables,  folio,  tipo_estregable,  nombre_entragable,id_cat_ejes_estrategicos,  id_cat_agendas,  a.descripcion,  liga_acceso,  no_isbn,  b.descripcion as nombre_eje,  c.descripcion as nombre_agenda
FROM entregables a
...
/*---------------------------------------------------------------------------------*/
/* Funcion para saber las cantidades de documentos en actuciones de medicion/Queja */
/*---------------------------------------------------------------------------------*/
...
function persEstadistica($tabla, $nombre_id, $nombre_campo)
{
  global $db;
  $sql  = "SELECT 
			IFNULL({$db->escape($nombre_campo)},'Sin Dato') as nombre, 
			COUNT(id_det_usuario) as total 
...
/*------------------------------------------------------------------------*/
/* Funcion para estadistica de personal-sueldos */
/*------------------------------------------------------------------------*/

function persSueldo($nombre_campo)
{
  global $db;
  $sql  = "SELECT
				COUNT(id_det_usuario) as total ,
				IFNULL({$db->escape($nombre_campo)},0) as monto
			FROM `detalles_usuario` 
			GROUP BY {$db->escape($nombre_campo)};";
  return find_by_sql($sql);
}


/*------------------------------------------------------------------------*/
/* Funcion para estadistica de personal-escolaridad */
/*------------------------------------------------------------------------*/

function persEscolaridad()
{
  global $db;
  $sql  = "SELECT
				COUNT(IFNULL(id_rel_detalle_usuario,1)) as total,
				IFNULL(c.descripcion,'Sin Dato') as nombre
			FROM `detalles_usuario` a
			LEFT JOIN rel_curriculum_academico b ON(a.`id_det_usuario`=b.`id_rel_detalle_usuario`)
			LEFT JOIN cat_escolaridad c ON(b.`grado`=c.`id_cat_escolaridad`) 
			GROUP BY grado;";
  return find_by_sql($sql);
}

/*------------------------------------------------------------------------*/
/* Funcion para estadistica de personal-conocimiento */
/*------------------------------------------------------------------------*/

function persConocimiento()
{
  global $db;
  $sql  = "SELECT
	COUNT(IFNULL(id_detalle_usuario,1)) as total, 
    IFNULL(c.descripcion,'Sin Dato') as nombre 
FROM `detalles_usuario` a 
LEFT JOIN rel_area_conocimiento b ON(a.`id_det_usuario`=b.`id_detalle_usuario`)  
LEFT JOIN cat_area_conocimiento c ON(b.`tipo_area`=c.`id_cat_area_con`)  
GROUP BY tipo_area;";
  return find_by_sql($sql);
}
...
/*------------------------------------------------------------------------*/
/* Funcion para estadistica de personal-carrera */
/*------------------------------------------------------------------------*/

function persCarrera()
{
  global $db;
  $sql  = "SELECT
    COUNT(IFNULL(id_rel_detalle_usuario,1)) as total,
    IFNULL(estudios,'Sin Dato') as nombre
FROM `detalles_usuario` a
LEFT JOIN rel_curriculum_academico b ON(a.`id_det_usuario`=b.`id_rel_detalle_usuario`)
GROUP BY estudios;";
  return find_by_sql($sql);
...
/*----------------------------------------------*/
/* Funcion que encuentra todos los informes especiales */
/*----------------------------------------------*/
function find_all_infoEspeciales()
{
  global $db;
  $results = array();
  $sql = "SELECT  id_informes_especiales,  folio,  ejercicio, tipo_informe, nombre_informe,derecho_involucrado, id_cat_ejes_estrategicos,  id_cat_agendas,  a.descripcion,  link_informe,  no_isbn,  b.descripcion as nombre_eje,  c.descripcion as nombre_agenda
FROM informes_especiales a
LEFT JOIN `cat_ejes_estrategicos` b USING(id_cat_ejes_estrategicos)
LEFT JOIN `cat_agendas` c USING(id_cat_agendas)";
  $result = find_by_sql($sql);
  return $result;
...
/*----------------------------------------------*/
/* Funcion que encuentra todas las actividades de la Unidad de Desaparecidos */
/*----------------------------------------------*/
function find_all_actividadesUD()
{
  global $db;
  $results = array();
  $sql = "SELECT
	id_actividades_ud,folio, b.descripcion as tipo_actividades_ud, fecha_actividad,
	IF(id_cat_victima_atendida is null,CONCAT(e.nombre_colectivo),CONCAT(c.nombre,' ',c.paterno,' ',c.materno)) as victima_atendida,
  CONCAT(d.nombre,' ',d.paterno,' ',d.materno) as persona_desaparecida, 
  nombre_solicitante,quien_atendio
...
LEFT JOIN colectivos_atendidos e USING(id_colectivos_atendidos)";
  $result = find_by_sql($sql);
  return $result;
}

/*----------------------------------------------*/
/* Funcion que encuentra los datos de la victima o desaparacido */
/*----------------------------------------------*/
function find_by_id_victima_desaparecido($table, $id, $nombre_id)
{
  global $db;
  $id = (int)$id;
  if (tableExists($table)) {
    $query = "SELECT 
		...
		IFNULL(edad,'') as edad,
		IFNULL(leer_escribir,'') as leer_escribir,
		IFNULL(b.descripcion,'') as genero,
		IFNULL(c.descripcion,'') as nacionalidad,
		IFNULL(d.descripcion,'') as entidad,
		IFNULL(e.descripcion,'') as municipio,
		IFNULL(f.descripcion,'') as escolaridad,
		IFNULL(g.descripcion,'') as ocupacion,
		IFNULL(h.descripcion,'') as grupo_vulnerable,
		IFNULL(i.descripcion,'') as discapacidad,
		IFNULL(j.descripcion,'') as comunidad		
	  FROM {$db->escape($table)} 
	  LEFT JOIN cat_genero b USING(id_cat_gen)
	  LEFT JOIN cat_nacionalidades c USING(id_cat_nacionalidad)
	  LEFT JOIN cat_entidad_fed d USING(id_cat_ent_fed)
	  LEFT JOIN cat_municipios e USING(id_cat_mun)
	  LEFT JOIN cat_escolaridad f USING(id_cat_escolaridad)
	  LEFT JOIN cat_ocupaciones g USING(id_cat_ocup)
	  LEFT JOIN cat_grupos_vuln h USING(id_cat_grupo_vuln)
...
function find_by_id_victima_colectivo($id)
{
  ...
  $query = "SELECT 
		...
		a.id_cat_ent_fed,
		a.id_cat_mun,
		IFNULL(nombre_colectivo,'') as nombre_colectivo,
		IFNULL(d.descripcion,'') as entidad,
		IFNULL(e.descripcion,'') as municipio,
		IFNULL(no_beneficiarios,0) as no_beneficiarios,
		IFNULL(asistentes_hombres,0) as asistentes_hombres,
		IFNULL(asistentes_mujeres,0) as asistentes_mujeres,
		IFNULL(asistentes_nobinario,0) as asistentes_nobinario,
		IFNULL(asistentes_otros,0) as asistentes_otros
	  FROM colectivos_atendidos a
	  LEFT JOIN cat_entidad_fed d USING(id_cat_ent_fed)
	  LEFT JOIN cat_municipios e USING(id_cat_mun)
...
{
  $sql = "SELECT DISTINCT IFNULL(folio_expediente,'0') as folio_expediente ,IFNULL(f.folio,'Sin Expediente') as folio  FROM paciente pa LEFT JOIN folios f ON f.id_folio = pa.folio_expediente ; ";
  $result = find_by_sql($sql);
  return $result;
}
...
function find_all_pacientes()
{
  ...
  gv.descripcion as grupo_vulnerable, pa.leer_escribir, com.descripcion as comunidad, pa.telefono, pa.email, pa.folio_expediente, pa.tipo_expediente, IFNULL(f.folio,'Sin Expediente') as folio";
...
function find_by_id_paciente($id)
{
  ...
  $sql = $db->query("SELECT pa.id_paciente, pa.nombre, pa.paterno, pa.materno, gen.descripcion as genero, pa.edad, mun.descripcion as municipio, 
                      ...
                      pa.municipio as id_municipio, pa.entidad as id_entidad, pa.escolaridad as id_escolaridad, pa.ocupacion as id_ocupacion, 
					  pa.discapacidad as id_discapacidad, pa.grupo_vulnerable as id_gv, pa.comunidad as id_comunidad,  f.folio
                      FROM paciente pa
...
function find_by_id_paciente2($folio)
{
  ...
  $sql = $db->query("SELECT pa.id_paciente, pa.nombre, pa.paterno, pa.materno, gen.descripcion as genero, pa.edad, mun.descripcion as municipio, 
                      ...
                      LEFT JOIN cat_nacionalidades nac ON nac.id_cat_nacionalidad = pa.nacionalidad
                      LEFT JOIN folios f ON f.id_folio = pa.folio_expediente
                      WHERE pa.id_paciente = '{$folio}' LIMIT 1");
  if ($result = $db->fetch_assoc($sql))
    return $result;
  else
    return null;
...
function find_ficha_completa($id_ficha)
{
  ...
  $sql = $db->query("SELECT f.folio,fun.descripcion as funcion,fo.folio as num_queja,f.ficha_adjunto,
	   ...
	   esc.descripcion as escolaridad,gv.descripcion as grupo_vulnerable, 
       p.nombre,p.paterno,p.materno,p.edad,gen.descripcion as genero,
       f.tipo_ficha,f.id_paciente
FROM fichas f
     LEFT JOIN cat_funcion fun ON f.id_cat_funcion = fun.id_cat_funcion
     LEFT JOIN area a2 ON a2.id_area = f.id_area_solicitante
     LEFT JOIN paciente p ON p.id_paciente = f.id_paciente
     LEFT JOIN folios fo ON fo.id_folio = p.id_paciente
...
/*----------------------------------------------*/
/* Funcion que encuentra todas las colaboraciones de la Unidad de Desaparecidos */
/*----------------------------------------------*/
function find_all_Colaboraciones($ejercicio)
{
  global $db;
  $results = array();
  $sql = "SELECT
...
WHERE CONCAT(IFNULL(	nombre	,''),IFNULL(	paterno	,''),IFNULL(	materno	,''),IFNULL(	id_cat_gen	,''),IFNULL(	edad	,''), IFNULL(	id_cat_nacionalidad	,''),
    ...
    IFNULL(	id_cat_disc	,''),IFNULL(	id_cat_comun	,'')) = '{$quejoso}' LIMIT 1");
  if ($result = $db->fetch_assoc($sql))
    return $result;
  else
    return null;
...
/*----------------------------------------------------------------*/
/* Funcion que encuentra todas las acciones de vinmculacion       */
/*----------------------------------------------------------------*/
function find_all_accionesV($year)
{
  $sql = "SELECT av.id_accionV, av.folio, av.fecha, av.lugar, av.nombre_actividad, av.descripcion, av.participantes, a.descripcion as inst_procedencia, 
...
/*----------------------------------------------------------------*/
/* Funcion que encuentra un acciones de vinmculacion       */
/*----------------------------------------------------------------*/
...
/*----------------------------------------------------------------*/
/* Funcion que encuentra un todos los datos de SErvicio Social    */
/*----------------------------------------------------------------*/
...
/*----------------------------------------------------------------*/
/* Funcion que encuentra los datos de  SErvicio Socia       */
/*----------------------------------------------------------------*/
function find_by_id_ss($id_ss)
{
  ...
  $sql = $db->query("SELECT ss.id_servicio_social, ss.modalidad, ss.nombre_prestador, ss.paterno_prestador, ss.materno_prestador, g.id_cat_gen, 
                    ...
                    esc.descripcion as escolaridad,  disc.descripcion as discapacidad,  gv.descripcion as gv, 
                     com.descripcion as comunidad, c.descripcion as carrera, ss.institucion, ss.fecha_inicio, ss.fecha_termino, 
                    ...
                    esc.id_cat_escolaridad, disc.id_cat_disc, gv.id_cat_grupo_vuln, com.id_cat_comun, c.id_cat_carrera,id_indicadores_pat,oficio_asignacion_area,oficio_obligaciones_prestador
                    FROM servicio_social ss
                    LEFT JOIN cat_carreras c ON c.id_cat_carrera = ss.carrera
...
function find_all_otras_acciones($year)
{
...
/*--------------------------------------------------------------*/
/* Funcion para mostrar las acciones relevantes del area se sistemas*/
/*--------------------------------------------------------------*/
function find_all_accionesCSI($year)
{
  $sql = "SELECT id_acciones_sistemas, a.id_cat_tipo_accion_sistemas,b.descripcion as tipo_accion, nombre_area, a.id_area,
			fecha_accion,descripcion_accion, quien_atendio
...
/*--------------------------------------------------------------*/
/* Funcion para contar las reclasificaciones de una queja*/
/*--------------------------------------------------------------*/
...
/*--------------------------------------------------------------*/
/* Funcion para mostrar las reclasificaciones de una queja*/
/*--------------------------------------------------------------*/
function find_reclasificacion_queja($id)
{
  global $db;
  $sql  = "SELECT id_rel_queja_reclasificacion,fecha_reclasificacion,observaciones_reclasificacion FROM rel_queja_reclasificacion WHERE id_queja_date=  '{$id}'";
  return $db->query($sql);
}
...
/*----------------------------------------------------------------------------------*/
/* Funcion que encuentra una correspondencia externa por id, que ayudara al momento de editar */
/*----------------------------------------------------------------------------------*/
function find_by_id_env_correspondenciaExter($id)
{
  global $db;
  $id = (int)$id;
  $sql = $db->query("SELECT id_correspondencia_externa,
	folio,
    num_oficio,
    fecha_oficio,
    nombre_destinatario,
	...
	oficio_enviado 
	FROM correspondencia_externa a
    WHERE id_correspondencia_externa='{$db->escape($id)}' LIMIT 1");
  if ($result = $db->fetch_assoc($sql))
    return $result;
  else
    return null;
...
/*----------------------------------------------------------------------------------*/
/* Funcion que encuentra el nombre del archivo pincipal de cada area */
/*----------------------------------------------------------------------------------*/
function find_by_solicitud($id)
{
  global $db;
  $id = (int)$id;
  $sql = $db->query("SELECT nombre_solicitud, nombre_area FROM area WHERE id_area='{$db->escape($id)}' LIMIT 1");
  if ($result = $db->fetch_assoc($sql))
    return $result;
  else
    return null;
...
/*----------------------------------------------------------------------------------*/
/* Funcion que encuentra los datos estadisticos de las quejas por estado procesal  */
/*----------------------------------------------------------------------------------*/
function estadoProcesal_estQ($year)
{
  global $db;
...
/*----------------------------------------------------------------------------------*/
/* Funcion que encuentra los datos estadisticos de las quejas por resolucion  */
/*----------------------------------------------------------------------------------*/
function resolucion_estQ($year)
{
  global $db;
...
/*--------------------------------------------------------------*/
/* Funcion para mostrar las acuerdo de orientaciones/canalizaciones
/*--------------------------------------------------------------*/

function find_acuerdo_orican($id)
{
  global $db;
  $sql  = "SELECT id_rel_orientacion_canalizacion_acuerdos,tipo_acuerdo,
			fecha_acuerdo,acuerdo_adjunto,acuerdo_adjunto_publico,
			sintesis_documento,publico
			FROM `rel_orientacion_canalizacion_acuerdos` 
			WHERE id_or_can= {$id} ORDER By fecha_acuerdo ASC";
  return $db->query($sql);
}

/*--------------------------------------------------------------*/
/* Funcion para mostrar saber si es una orientaciones/canalizaciones
/*--------------------------------------------------------------*/

function find_by_tipo($id)
{
  global $db;
  $id = (int)$id;
  $sql = $db->query("SELECT o.id_or_can,o.folio,nombre_completo,o.institucion_canaliza,o.medio_presentacion,
					tipo_solicitud,med.descripcion as med,aut.nombre_autoridad as aut
                      FROM orientacion_canalizacion as o  
                      LEFT JOIN cat_autoridades aut ON aut.id_cat_aut = o.institucion_canaliza
					  LEFT JOIN cat_medio_pres med ON med.id_cat_med_pres = o.medio_presentacion
  WHERE id_or_can='{$db->escape($id)}' LIMIT 1");
  if ($result = $db->fetch_assoc($sql))
    return $result;
  else
    return null;
...
/*--------------------------------------------------*/
/* Funcion que encuentra los ehercicios de las recomendaciones emitidas */
/*--------------------------------------------------*/
function find_all_year_recos()
{
  $sql = "SELECT
	SUBSTRING(numero_recomendacion,5) as years
FROM recomendaciones a
GROUP by SUBSTRING(numero_recomendacion,5)
ORDER by SUBSTRING(numero_recomendacion,5);";
  $result = find_by_sql($sql);
  return $result;
}
...
/*--------------------------------------------------------------*/
/* Funcion para encontrar en una tabla toda la informacion de un curso
/*--------------------------------------------------------------*/
...
b.`descripcion` as nombre_eje,
id_cat_agendas,
c.`descripcion` as nommbre_agenda,
nombre_curso,
objetivo,
a.descripcion,
fecha_apertura,
...
nombre_responsable,
id_cat_modalidad,
e.`descripcion` as nombre_modalidad,
fecha_tecnica,
expediente_tecnico,
observaciones,
id_cat_tipo_actividad,
f.`descripcion` as tipo_actividad,
id_cat_categoria_actividad,
g.`descripcion` as nombre_categoria

FROM cursos_diplomados a
LEFT JOIN cat_ejes_estrategicos b USING(id_cat_ejes_estrategicos)
LEFT JOIN cat_agendas c USING(id_cat_agendas)
...
/*--------------------------------------------------------------*/
/* Funcion para mostrar los datos del curso/diplomado
/*--------------------------------------------------------------*/

function find_by_id_curso($id)
{
  global $db;
  $id = (int)$id;
  $sql = $db->query("SELECT 
...
LEFT JOIN cat_tipo_actividad f USING(id_cat_tipo_actividad)
LEFT JOIN cat_categoria_actividad g USING(id_cat_categoria_actividad)
  WHERE id_cursos_diplomados='{$db->escape($id)}' LIMIT 1");
  if ($result = $db->fetch_assoc($sql))
    return $result;
  else
    return null;
...
function find_all_pat($area)
{
 ...
 id_indicadores_pat,
  ...
  id_cat_unidades_medida ,
  c.descripcion as unidades_medida,
  valor_absoluto ,
  table_relacionada,
  campo_relacionado,
  fecha_relacionada,
...
WHERE id_area_responsable={$area};";
  $result = find_by_sql($sql);
  return $result;
}
...
/*--------------------------------------------------------------*/
/* Funcion para encontrar en una tabla toda la informacion de certificaciones
/*--------------------------------------------------------------*/
...
avance_proceso,
emisor_certificacion,
contacto_certificacion,
id_area_responsable,
d.nombre_area,
nombre_responsable
FROM certificaciones a
LEFT JOIN cat_ejes_estrategicos b USING(id_cat_ejes_estrategicos)
LEFT JOIN cat_agendas c USING(id_cat_agendas)
LEFT JOIN area d ON(a.id_area_responsable=d.id_area) ");
}
...
function find_by_id_certificacion($id)
{
  global $db;
  $id = (int)$id;
  $sql = $db->query("SELECT 
...
LEFT JOIN cat_agendas c USING(id_cat_agendas)
LEFT JOIN area d ON(a.id_area_responsable=d.id_area)
  WHERE id_certificaciones='{$db->escape($id)}' LIMIT 1");
  if ($result = $db->fetch_assoc($sql))
    return $result;
  else
    return null;
...
id_indicadores_pat,
tipo,
SUM(valor_enero) as valor_enero,
SUM(valor_febrero) as valor_febrero,
SUM(valor_marzo) as valor_marzo,
SUM(valor_abril) as valor_abril,
SUM(valor_mayo) as valor_mayo,
SUM(valor_junio) as valor_junio,
SUM(valor_julio) as valor_julio,
SUM(valor_agosto) as valor_agosto,
SUM(valor_septiembre) as valor_septiembre,
SUM(valor_octubre) as valor_octubre,
SUM(valor_noviembre) as valor_noviembre,
...
function find_all_colecciones()
{
  ...
  nombre_responsable,
  tipo_publicacion,
  temporalidad_proyecto,
  hipervinculo_proyecto,
  observaciones
...
FROM `colecciones_estudios` a
LEFT JOIN area b ON(id_area= id_area_responsable)
WHERE id_colecciones_estudios='{$db->escape($id)}'  LIMIT 1");
  if ($result = $db->fetch_assoc($sql))
    return $result;
  else
    return null;
...
function find_all_pat_area($area, $table_relacionada)
{
 ...
 id_indicadores_pat,
  ...
  nombre_indicador 
FROM indicadores_pat a
WHERE id_area_responsable={$area}   AND table_relacionada='{$table_relacionada}';";
  $result = find_by_sql($sql);
  return $result;
}
...
{
  $sql = "SELECT id_contactos_politicas,nombres,apellidos, gen.descripcion as genero,cargo_desempelado,telefono,email,nombre_autoridad ";
  $sql .= " FROM contactos_politicas pa";
  $sql .= " LEFT JOIN cat_genero gen ON gen.id_cat_gen = pa.id_cat_gen";
  $sql .= " LEFT JOIN cat_autoridades ent ON ent.id_cat_aut = pa.id_cat_aut";
  $result = find_by_sql($sql);
  return $result;
}
...
{
  $sql = "SELECT  id_cat_aut,nombre_autoridad
FROM `contactos_politicas` a
LEFT JOIN `cat_autoridades` b USING(id_cat_aut)
GROUP BY id_cat_aut; ";
  $result = find_by_sql($sql);
  return $result;
}
...
function find_oficios_monitoreo($id, $tipo)
{
  ...
  c.id_cat_aut,
nombre_autoridad,
CONCAT(nombres,' ',apellidos) as nombre_contacto,
id_rel_monitoreo_oficios,
id_monitoreo_politicas,
id_contactos_politicas,
documento_oficio,
tipo_documento
...
/*--------------------------------------------------------------*/
/* Funcion para mostrar los derechos actuales de una queja*/
/*--------------------------------------------------------------*/
function find_by_violentadosAct($tableA, $tableB, $id)
{
  global $db;
  $id = (int)$id;
  $sql = "SELECT id_{$db->escape($tableB)} ,IFNULL(descripcion,'')as descripcion 
...
/* Funcion que encuentra los indicadores del PAT */
/*--------------------------------------------------*/
function find_all_pat_corto($area)
{
  $sql = "SELECT
 id_indicadores_pat,
  id_area_responsable,
...
/*--------------------------------------------------------------*/
/* Funcion para encontrar  REGISTRO DE ATENCIÓN/SEGUIMIENTO 
/*--------------------------------------------------------------*/
function find_all_atencion_seguimiento($id_area)
{
 ...
 id_atencion_seguimiento,
  ...
  mes,
  ejercicio,
  SUM(numero_accion) as numero_accion
 FROM `atencion_seguimiento` a
 LEFT JOIN  rel_accion_atencion USING(id_atencion_seguimiento)
...
/*--------------------------------------------------------------*/
/* Funcion para encontrar un REGISTRO DE ATENCIÓN/SEGUIMIENTO 
/*--------------------------------------------------------------*/
...
/*--------------------------------------------------------------*/
/* Funcion para encontrar las areas de los usuarioas que an creado orientaciones/canalizaciones 
/*--------------------------------------------------------------*/
function find_all_areaCreacion($tipo)
{
  $sql = "SELECT
	c.id_area,
    d.`nombre_area`
FROM orientacion_canalizacion 
left join `users` a ON(`id_user`=id_creador)
LEFT JOIN `detalles_usuario` b ON(id_detalle_user=id_det_usuario)
LEFT JOIN cargos c ON(id_cargo = id_cargos)
LEFT JOIN `area` d ON(c.id_area = d.id_area)  
WHERE tipo_solicitud={$tipo}
GROUP by c.id_area  
ORDER BY d.`jerarquia`";
  $result = find_by_sql($sql);
  return $result;
}

/*--------------------------------------------------------------*/
/* Funcion para encontrar los modulos que teien cada area
/*--------------------------------------------------------------*/
...
/*--------------------------------------------------------------*/
/* Funcion para encontrar totales en correspondencia
/*--------------------------------------------------------------*/
function find_all_correspondencia_totales()
{
  $sql = "SELECT 
COUNT(id_correspondencia) as total_registros,
YEAR(fecha_recibido) as ejercicio
FROM correspondencia a     
GROUP By YEAR(fecha_recibido)     
ORDER BY YEAR(fecha_recibido) ;";
  $result = find_by_sql($sql);
  return $result;
}
...
function find_all_prestamos_vehiculos()
{
  $sql = "SELECT rpv.id_rel_prestamo_vehiculo, rpv.fecha_hora_presta, rpv.fecha_hora_regresa, CONCAT(v.marca, ' ', v.modelo) as vehiculo, v.placas, 
          CONCAT(d.nombre, ' ', d.apellidos) as nombre
          FROM rel_prestamo_vehiculo rpv
          LEFT JOIN vehiculos v ON v.id_vehiculo = rpv.id_vehiculo
          LEFT JOIN detalles_usuario d ON d.id_det_usuario = rpv.id_detalle_usuario
          ORDER BY rpv.fecha_hora_presta DESC";
  $result = find_by_sql($sql);
  return $result;
}

function find_by_id_prestamo($id)
{
  global $db;
  $id = (int)$id;
  $sql = $db->query("SELECT rpv.id_rel_prestamo_vehiculo, rpv.fecha_hora_presta, rpv.fecha_hora_regresa, rpv.km_inicial, rpv.km_final, rpv.motivo_prestamo,
                      rpv.observaciones, CONCAT(v.marca, ' ', v.modelo) as vehiculo, v.placas,  CONCAT(d.nombre, ' ', d.apellidos) as nombre, a.nombre_area
                      FROM rel_prestamo_vehiculo rpv
                      LEFT JOIN vehiculos v ON v.id_vehiculo = rpv.id_vehiculo
                      LEFT JOIN detalles_usuario d ON d.id_det_usuario = rpv.id_detalle_usuario
                      LEFT JOIN area a ON a.id_area = d.id_area
                      WHERE rpv.id_rel_prestamo_vehiculo = '{$db->escape($id)}'");
  if ($result = $db->fetch_assoc($sql))
    return $result;
  else
    return null;
