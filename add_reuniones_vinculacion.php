...
if (isset($_POST['add_reuniones_vinculacion'])) {
    if (empty($errors)) {
        ...
        $id_indicadores_pat   = $db->escape($_POST['id_indicadores_pat']);
        ...
        $query .= "folio, nombre_reunion, fecha_reunion, lugar_reunion, modalidad, numero_asistentes, alcances_acuerdos, observaciones, lista_asistencia,
                    id_indicadores_pat, area, id_user_creador, fecha_creacion";
        $query .= ") VALUES (";
        $query .= "'{$folio}', '{$nombre_reunion}', '{$fecha_reunion}', '{$lugar_reunion}', '{$modalidad}', '{$numero_asistentes}', '{$alcances_acuerdos}',
                    '{$observaciones}', '{$lista_asistencia}', '{$id_indicadores_pat}', '{$tipo}', '{$id_user}', NOW()";
        $query .= ")";
        ...
        for ($i = 0; $i < sizeof($nombre_participante); $i = $i + 1) {
            if ($nombre_participante[$i] !== '') {
                $query = "INSERT INTO rel_reuniones_vinculacion_asistentes (id_reuniones_vinculacion, nombre_participante,procedencia_participante) 
                            VALUES($id_reuniones_vinculacion, '$nombre_participante[$i]','$procedencia_participante[$i]');";
                if ($db->query($query)) {
        ...
        redirect('add_reuniones_vinculacion.php?t=' . $tipo, false);
    }
}
?>
...
<div class="row">
    <div class="panel panel-default">
        ...
        <div class="panel-body">
            <form method="post" action="add_reuniones_vinculacion.php?t=<?php echo $tipo ?>" enctype="multipart/form-data">
                <div class="row">
                    <div class="col-md-2">
                ...
                <div class="form-group clearfix">
                    <a href="reuniones_vinculacion.php?t=<?php echo $tipo ?>" class="btn btn-md btn-success" data-toggle="tooltip" title="Regresar">
                        Regresar
                    </a>
